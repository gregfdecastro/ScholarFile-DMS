...
include("connection.php");
    ...
    if (isset($_POST['upload'])) {
        ...
        if (in_array($file_type, $allowTypes)) {
            $insert_query = mysqli_query($con, "INSERT INTO `quizzes`(title, uploaded_by, file) VALUES('$title', '$uploaded_by', '$file')") or die('query failed');
    ...
    if (isset($_GET['delete'])) {
        ...
        $delete_query = mysqli_query($con, "DELETE FROM `quizzes` WHERE qid = $delete_id ") or die('query failed');
        if ($delete_query) {
            header('location:quiz.php');
 ...
 if (isset($_POST['update'])) {
    ...
    if (in_array($file_type, $allowTypes)) {
        $update_query = mysqli_query($con, "UPDATE `quizzes` SET title = '$update_title', file = '$update_file', uploaded_by = '$update_uploader' WHERE qid = '$update_id'");
