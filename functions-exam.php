...
include("connection.php");
   ...
   if (isset($_POST['upload'])) {
    ...
    if (in_array($file_type, $allowTypes)) {
        $insert_query = mysqli_query($con, "INSERT INTO `exams`(title, uploaded_by, file) VALUES('$title', '$uploaded_by', '$file')") or die('query failed');
        ...
        if ($title == '' or $file == '') {
            ...
            header('location:exam-create.php');
        }
    }
}
;
...
if (isset($_GET['delete'])) {
    ...
    $delete_query = mysqli_query($con, "DELETE FROM `exams` WHERE eid = $delete_id ") or die('query failed');
    if ($delete_query) {
        header('location:exam.php');
...
if (in_array($file_type, $allowTypes)) {
    $update_query = mysqli_query($con, "UPDATE `exams` SET title = '$update_title', file = '$update_file', uploaded_by = '$update_uploader' WHERE eid = '$update_id'");
